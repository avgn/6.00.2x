...
class Cluster(object):
    ...
    def singleLinkageDist(self, other):
        ...
        dist = list(self.points)[0].distance(list(other.points)[0])
        for point in self.points:
            for otherPoint in other.points:
                if point.distance(otherPoint) < dist:
    ...
    def averageLinkageDist(self, other):
        ...
        for point in self.points:
            for otherPoint in other.points:
                distances.append(point.distance(otherPoint))
        totDist = 0.0
        for dist in distances:
            totDist += dist
        return totDist/float(len(distances))
...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        mergedCluster = Cluster(c1.points + c2.points, c1.pointType)
        self.members.remove(c1)
        self.members.remove(c2)
        self.members.append(mergedCluster)
    ...
    def findClosest(self, linkage):
        ...
        for c1 in self.members:
            for c2 in self.members:
                if c1 != c2:
                    if linkage(c1, c2) < linkage(closest[0], closest[1]):
                        closest = (c1, c2)
        return closest
