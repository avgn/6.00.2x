...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    avgPopSize = [0.0 for i in range(300)]
    ...
    pylab.title("Average Size of the Virus Population")
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        if reproducible:
            if random.random() <= self.maxBirthProb * (1 - popDensity):
                offspringResistances = dict(self.resistances)
                for resistance in offspringResistances:
                    if random.random() <= self.mutProb:
                        offspringResistances[resistance] = not self.resistances[resistance]
                offspring = ResistantVirus(self.maxBirthProb, self.clearProb, offspringResistances, self.mutProb)
                return offspring
            else:
                raise NoChildException
        else:
...
class TreatedPatient(Patient):
    ...
    def getResistPop(self, drugResist):
        ...
        resistantViruses = []
        for virus in self.viruses:
            virusResistance = True
            for drug in drugResist:
                if not virus.isResistantTo(drug):
                    virusResistance = False
                    break
            if virusResistance:
    ...
    def update(self):
        ...
        for virus in self.viruses[:]:
            if len(self.viruses) < self.maxPop:
                try:
                    offspring = virus.reproduce(popDensity, self.activeDrugs)
                    self.viruses.append(offspring)
                    popDensity = len(self.viruses) / float(self.maxPop)
                except NoChildException:
                    pass
